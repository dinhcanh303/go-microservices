...
func (g *groupGRPCClient) GetAllGroupIdByUserId(ctx context.Context, userId uuid.UUID) ([]uuid.UUID, error) {
	client := gen.NewGroupServiceClient(g.conn)

	res, err := client.GetAllGroupIdByUserId(ctx, &gen.GetAllGroupIdByUserIdRequest{
